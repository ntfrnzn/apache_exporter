...
type Exporter struct {
	...
	kiloBytesTotal *prometheus.Desc
...
func NewExporter(uri string) *Exporter {
	return &Exporter{
		...
		accessesTotal: prometheus.NewDesc(
			...
			nil,
			nil),
		kiloBytesTotal: prometheus.NewDesc(
...
func (e *Exporter) updateScoreboard(scoreboard string) {
	e.scoreboard.Reset()
	for _, workerStatus := range scoreboard {
		s := string(workerStatus)
		switch {
		case s == "_":
...
func (e *Exporter) collect(ch chan<- prometheus.Metric) error {
	...
	for _, l := range lines {
		...
		case key == "Total kBytes":
			...
			ch <- prometheus.MustNewConstMetric(e.kiloBytesTotal, prometheus.CounterValue, val)
		case key == "Uptime":
			val, err := strconv.ParseFloat(v, 64)
			if err != nil {
				return err
